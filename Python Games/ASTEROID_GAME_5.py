...
import sys
pygame.init()
recharge = pygame.time.get_ticks()
delay = 1000
...
player_sizey = 50 #player variables
player_sizex = 50
player_posi = [width/2, height-2*player_sizey]
player_speed = 11
lives = 3

asteroid_size = 50  #Asteroid variables
...
def firebeam(beamX,beamY):
        global beam_state
        beam_state = "fire"
        pygame.draw.rect(screen, red, (beamX+30, beamY+10, 6, 32))
...
while not game_over:
        ...
        if keys[pygame.K_SPACE]:
                if beam_state is "ready" and pygame.time.get_ticks()-recharge >= delay:
                        recharge = pygame.time.get_ticks()
                        beamX = player_posi[0]
                        beamY = player_posi[1]
        ...
        text3 = "space to fire"
        label = myfont.render(text3, 1, yellow)
        screen.blit(label, (2, height-20))
